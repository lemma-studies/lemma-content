...
*Amendment, 2026-09-11.* Readers in Arabic, Persian and Hebrew are served by a modern Bible as well as the public-domain one. The best openly licensed modern versions in these languages are Biblica's Open editions (NAV, Open Persian Contemporary Bible, Hebrew Living NT), and they are CC-BY-SA 4.0. For the Arabic and Persian texts, Biblica replied on 2026-08-31 that they need no permission request or signed agreement, are used under CC-BY-SA, and need no further action from Biblica; the reply offered no other terms. The Hebrew Living NT's licence rests on its own published copyright page.
...
- **Enforced in code (2026-09-11).** `verify-study-yaml.mjs` rejects a study whose `translation_of.scripture_anchor` names an upgrade candidate unless `study.license` equals the candidate's `edition_license` and, while the candidate is held, `rights_tier` is `private_pending_grant`; it also rejects `cc-by-sa-4.0` on a study with no such anchor. `verify-release.mjs` now runs that schema check (`study_yaml_schema`) and fails any study at `private_pending_grant` (`rights_tier_publishable`). `zenodo-reserve-doi.mjs`, `update-huggingface-ds.mjs` and `regenerate-corpus.mjs` take the licence from `study.yaml` through `scripts/lemma-cli/_licence.mjs` (no CC-BY default; an unknown or missing licence stops them), refuse or skip a held study, and are covered by `npm run self-test`.
- **Still held.** The site still states CC-BY 4.0 for every page: `site/src/components/Head.astro` (JSON-LD `license`, `DC.rights`), `site/public/.well-known/ai.txt`, `site/public/llms.txt` and `robots.txt`. Until the site takes a page's licence from its study, the open-licence candidates stay `rights_tier_when_used: private_pending_grant` (`publish_blocked_until: edition-licence-enforcement`), and the code above keeps any study anchored to them from publishing.
